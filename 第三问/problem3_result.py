...
df_raw = pd.read_pickle('output/merged_data.pkl')
df_wf = pd.read_pickle('output/dispatch_waterfill.pkl')
df_uniform = pd.read_pickle('output/dispatch_uniform.pkl')
prep = np.load('output/preprocess_data.npz', allow_pickle=True)
...
df_table_a.to_excel('output/表A_调度前后峰谷差对比.xlsx', index=False)
print('  表A 已保存')
...
df_table_b.to_excel('output/表B_过载风险评估.xlsx', index=False)
print('  表B 已保存')
...
fig1.savefig('output/图15_全市调度前后负荷曲线对比.png', dpi=300, bbox_inches='tight')
plt.close()
print('  图15 已保存')
...
plt.tight_layout(rect=[0, 0.06, 1, 1])
fig2.savefig('output/图16_各区域峰谷差降低率.png', dpi=300, bbox_inches='tight')
plt.close()
print('  图16 已保存')
...
plt.tight_layout(rect=[0, 0.06, 1, 1])
fig3.savefig('output/图17_各区域分面负荷曲线.png', dpi=300, bbox_inches='tight')
plt.close()
print('  图17 已保存')
...
plt.tight_layout(rect=[0, 0.06, 1, 1])
fig18.savefig('output/图18_各区域峰谷差对比.png', dpi=300, bbox_inches='tight')
plt.close()
print('  图18 已保存')
...
plt.tight_layout(rect=[0, 0.06, 1, 1])
fig19.savefig('output/图19_过载风险消除效果.png', dpi=300, bbox_inches='tight')
plt.close()
print('  图19 已保存')
