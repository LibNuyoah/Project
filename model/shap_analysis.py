...
def prepare_features(df):
    ...
    # 导入共享的特征工程函数
    from xgboost_model import prepare_features as pf
...
def plot_shap_results(shap_values, X_sample, feature_names, shap_importance):
    ...
    if shap_values is None:
        ...
        plt.tight_layout()
        output_path = os.path.join(RESULT_DIR, 'figures', 'shap_summary.png')
...
def main():
    ...
    shap_importance.to_excel(
        os.path.join(RESULT_DIR, 'tables', 'shap_importance.xlsx'), index=False
    )
    print(f"✅ SHAP特征重要性已保存: {os.path.join(RESULT_DIR, 'tables', 'shap_importance.xlsx')}")
