...
from utils.paths import RESULT_DIR, RESULT_FIGURES, RESULT_TABLES, DATA_RAW
...
def load_region_data():
    """加载附件1区域基础数据"""
    filepath = os.path.join(DATA_RAW, '附件 1 市主城区 10 个典型区域基础数据.xlsx')
...
def plot_cluster_results(df, X_scaled, best_k, inertias, sil_scores, K_range,
    ...
    plt.tight_layout()
    output_path = os.path.join(RESULT_FIGURES, 'cluster_analysis.png')
...
def main():
    ...
    df[output_cols].to_excel(
        os.path.join(RESULT_TABLES, 'cluster_result.xlsx'), index=False
    )
    print(f"✅ 聚类结果已保存: {os.path.join(RESULT_TABLES, 'cluster_result.xlsx')}")
