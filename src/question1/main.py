...
from utils.paths import (
    RESULT_DIR, RESULT_FIGURES, RESULT_TABLES, MODEL_DIR,
    FILE_CLEAN_DATA, FILE_PREDICTION_RESULT
)
...
def load_model():
    """加载训练好的XGBoost模型"""
    model_path = os.path.join(ROOT, 'model', 'xgboost_model.pkl')
...
def create_model_comparison():
    ...
    xgb_metrics = pd.read_excel(os.path.join(RESULT_TABLES, 'xgboost_metrics.xlsx'))
    ...
    # 保存
    comparison.to_excel(os.path.join(RESULT_TABLES, 'model_comparison.xlsx'), index=False)
    print(f"\n✅ 模型评价表已保存: {os.path.join(RESULT_TABLES, 'model_comparison.xlsx')}")
...
def plot_final_summary(daily_summary, comparison, pred_df):
    ...
    plt.tight_layout()
    output_path = os.path.join(RESULT_FIGURES, 'prediction_summary.png')
...
def main():
    ...
    # 保存详细分时段预测
    pred_df.to_excel(os.path.join(RESULT_TABLES, 'hourly_prediction.xlsx'), index=False)
    print(f"✅ 分时段预测已保存: {os.path.join(RESULT_TABLES, 'hourly_prediction.xlsx')}")
