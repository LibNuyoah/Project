...
from utils.paths import (
    RESULTS_Q3, RESULTS_FIGURES, FILE_Q3_MERGED_DATA, FILE_Q3_DISPATCH_WATERFILL,
    FILE_Q3_DISPATCH_UNIFORM, FILE_Q3_PREPROCESS, FILE_Q3_TABLE_A, FILE_Q3_TABLE_B
)
OUTPUT_DIR = RESULTS_Q3
...
df_raw = pd.read_pickle(FILE_Q3_MERGED_DATA)
df_wf = pd.read_pickle(FILE_Q3_DISPATCH_WATERFILL)
df_uniform = pd.read_pickle(FILE_Q3_DISPATCH_UNIFORM)
prep = np.load(FILE_Q3_PREPROCESS, allow_pickle=True)
...
for dtype in ['工作日', '周末']:
    ...
    city_after = df_a.groupby('小时')['调度后负荷'].sum()
    ma = compute_metrics(city_after.values, city_grid.values)
    city_results[dtype] = {'before': mb, 'after': ma, 'after_load': city_after}
...
for dtype in ['工作日', '周末']:
    r = city_results[dtype]
    reduction = r['before']['delta_p'] - r['after']['delta_p']
    reduction_rate = reduction / r['before']['delta_p'] * 100
    print(f"\n{dtype}:")
    print("调度前峰谷差:", r['before']['delta_p'])
    print("调度后峰谷差:", r['after']['delta_p'])
    print("峰谷差下降:", reduction)
    print("下降比例:", reduction_rate)
    print("调度前负荷率:", r['before']['load_rate'])
    print("调度后负荷率:", r['after']['load_rate'])
    print("24小时优化后负荷:")
    print(r['after_load'].values)

print('\n' + '=' * 60)
print('problem3_result.py 完成！')
print('=' * 60)
