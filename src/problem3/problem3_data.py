...
输出：
  - results/q3_output/表_调度前分析.xlsx
...
from utils.paths import (
    FILE_ATTACHMENT3, FILE_ATTACHMENT4,
...
PRICE_PEAK   = 1.19; PRICE_FLAT   = 0.70; PRICE_VALLEY = 0.21
...
print(f'  附件3长表: {df3_long.shape}')

# =============================================================================
# 2. 读取附件4
# =============================================================================
print('\n[2/4] 读取附件4...')
df4_raw = pd.read_excel(FILE_ATTACHMENT4)
...
for i in range(10):
    ...
    for h, tc in enumerate(time_cols4):
        records4.append({
            '区域编号': rid, '小时': h, '电网允许负荷': float(df4_raw.iloc[i][tc])
        })
...
for dtype in ['工作日', '周末']:
    ...
    m['区域编号'] = 0; m['区域名称'] = '全市汇总'; m['日期类型'] = dtype
    city_rows.append(m)
...
df_table.to_excel(FILE_Q3_BEFORE_ANALYSIS, index=False)
print(f'\n表已保存: {FILE_Q3_BEFORE_ANALYSIS}')

np.savez(FILE_Q3_PREPROCESS,
...
df.to_pickle(FILE_Q3_MERGED_DATA)
