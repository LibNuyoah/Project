...
from utils.paths import RESULTS_DIR, RESULTS_FIGURES, RESULTS_TABLES, MODELS_DIR, FILE_CLEAN_DATA, FILE_XGBOOST_MODEL, FILE_SHAP_IMPORTANCE
...
def plot_shap_results(shap_values, X_sample, feature_names, shap_importance):
    ...
    if shap_values is None:
        ...
        plt.tight_layout()
        output_path = os.path.join(RESULTS_FIGURES, 'shap_summary.png')
...
def main():
    ...
    shap_importance.to_excel(
        FILE_SHAP_IMPORTANCE, index=False
    )
    print(f"SHAP特征重要性已保存: {FILE_SHAP_IMPORTANCE}")
    ...
    print("SHAP关键指标")
    print("=" * 60)
    print("TOP1特征:", top3['特征'].iloc[0])
    print("TOP1 SHAP重要性:", top3['SHAP重要性'].iloc[0])
    print("TOP2特征:", top3['特征'].iloc[1])
    print("TOP2 SHAP重要性:", top3['SHAP重要性'].iloc[1])
    print("TOP3特征:", top3['特征'].iloc[2])
    print("TOP3 SHAP重要性:", top3['SHAP重要性'].iloc[2])
