...
def load_region_data():
    """加载附件1区域基础数据"""
    df = pd.read_excel(FILE_ATTACHMENT1).iloc[:10].copy()
    df.columns = [
...
def plot_cluster_results(df, X_scaled, best_k, inertias, sil_scores, K_range,
    ...
    plt.tight_layout()
    output_path = os.path.join(RESULTS_FIGURES, 'cluster_analysis.png')
...
def main():
    ...
    df[output_cols].to_excel(
        FILE_CLUSTER_RESULT, index=False
    )
    print(f"✅ 聚类结果已保存: {FILE_CLUSTER_RESULT}")
