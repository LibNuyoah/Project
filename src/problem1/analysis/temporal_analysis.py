...
from utils.paths import RESULTS_DIR, RESULTS_FIGURES, FILE_CLEAN_DATA
from src.problem1.analysis.region_type_loader import get_region_names
...
def plot_temporal(hourly_analysis):
    ...
    plt.tight_layout()
    output_path = os.path.join(RESULTS_FIGURES, 'temporal.png')
...
def plot_weekday_weekend(daily_pivot, hourly_pivot):
    ...
    plt.tight_layout()
    output_path = os.path.join(RESULTS_FIGURES, 'weekday_weekend.png')
    fig.savefig(output_path, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
...
def main():
    ...
    print("时间维度关键变量")
    print("=" * 60)
    print("工作日峰值时间:", peak_wd)
    print("工作日谷值时间:", valley_wd)
    print("周末峰值时间:", peak_we)
    print("工作日峰谷比:", hourly_pivot['工作日'].max() / hourly_pivot['工作日'].min())
    print("工作日/周末平均差异率(%):", avg_diff)
    print("各区域工作日需求:")
    print(daily_pivot[['区域名称', '工作日', '周末', '差异率(%)']].to_string())
