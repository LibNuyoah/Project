...
from utils.paths import RESULTS_DIR, RESULTS_FIGURES, RESULTS_TABLES, FILE_CLEAN_DATA, FILE_CORRELATION_MATRIX
...
def plot_correlation_heatmap(corr_matrix):
    ...
    plt.tight_layout()
    output_path = os.path.join(RESULTS_FIGURES, 'correlation_heatmap.png')
...
def plot_factor_bar(demand_corr):
    ...
    plt.tight_layout()
    output_path = os.path.join(RESULTS_FIGURES, 'factor_correlation.png')
    fig.savefig(output_path, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
...
def main():
    ...
    # 保存相关性矩阵
    corr_matrix.to_excel(FILE_CORRELATION_MATRIX)
    print(f"\n相关性矩阵已保存: {FILE_CORRELATION_MATRIX}")
    ...
    print("=" * 60)
    print("TOP5影响因素:")
    print(demand_corr.abs().sort_values(ascending=False).head(5).to_string())
