...
def plot_spatial(analysis):
    ...
    plt.tight_layout()
    output_path = os.path.join(RESULTS_FIGURES, 'spatial.png')
...
def main():
    ...
    print("空间维度关键变量")
    print("=" * 60)
    print("最大需求区域:", max_demand['区域名称'])
    print("最大需求值:", max_demand['日均需求'])
    print("最小需求区域:", min_demand['区域名称'])
    print("最小需求值:", min_demand['日均需求'])
    print("区域需求比:", ratio)
    print("各区域日均需求:")
    print(analysis[['区域名称', '日均需求', '单桩日均车次']].to_string(index=False))
