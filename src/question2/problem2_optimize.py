"""
=============================================================================
problem2_optimize.py — 问题二：基于NSGA-II的多目标充电桩布局优化（修正版）
=============================================================================
功能：
  1. 建立三目标优化模型（成本、地理覆盖率、负荷均衡）
  2. 覆盖率模型修正：基于附件5定义——充电覆盖面积/区域总面积
  3. 使用NSGA-II算法求解Pareto最优解集

修正说明：
  覆盖率 = 充电覆盖面积 / 区域总面积（附件5原文定义）
  新增充电桩通过扩大充电覆盖面积来提高覆盖率。
  简化模型：单桩边际覆盖面积 = πR² × (1-当前覆盖率)，考虑重叠递减。
...
#   每个新增充电桩可扩大充电覆盖面积。
#   单桩理论覆盖面积 = π × R²
#   但由于与现有桩位重叠，实际边际覆盖面积递减：
#   边际覆盖 = πR² × (1 - 当前覆盖率)
#   新增后覆盖率 = min(当前覆盖率 + Σ边际覆盖 / 总面积, 1.0)

SINGLE_COVERAGE_AREA = np.pi * region_radius**2  # 单桩理论覆盖面积 (km²)
...
def compute_coverage(delta_fast, delta_slow):
    ...
    每新增一个充电桩（无论快慢），新增覆盖面积 = πR² × (1-当前覆盖率)^β
    其中β>1体现重叠递减效应
    """
    ...
    for i in range(N_REGIONS):
        ...
        # 每个新桩的边际覆盖面积：理论面积 × 未覆盖率^1.5 (重叠递减)
        ...
        for j in range(N_REGIONS):
            if i != j and spillover_matrix[j, i] > 0.05:  # 仅考虑显著溢出
                spillover_contribution += (spillover_matrix[j, i] *
                                           delta_total[:, j] *
                                           SINGLE_COVERAGE_AREA[j] * 0.1)  # 跨区域贡献打折
...
def check_constraints(delta_fast, delta_slow):
    ...
    # 注：2100kW为附件5单站过载阈值，非区域总负荷上限
    # 区域总负荷约束由电网容量（15-35万kW）保证，此处不重复约束
...
# =============================================================================
# 4. NSGA-II算法（同前，略去重复注释）
# =============================================================================


def initialize_population():
    pop = np.zeros((POP_SIZE, N_VARS))
    # 根据紧迫度分配初始解：紧迫度高的区域多建桩
        ...
        if value_range > 0:
            for i in range(1, n - 1):
                distance[sorted_idx[i]] += (
                    values_sorted[i + 1] - values_sorted[i - 1]
                ) / value_range
    return distance
...
def sbx_crossover(p1, p2):
    ...
    for i in range(N_VARS):
        if np.random.random() < 0.5:
            ...
            v1 = np.clip(np.round(0.5*((1+beta)*p1[i] + (1-beta)*p2[i])), 0, MAX_NEW_PER_REGION)
            v2 = np.clip(np.round(0.5*((1-beta)*p1[i] + (1+beta)*p2[i])), 0, MAX_NEW_PER_REGION)
            c1[i], c2[i] = v1, v2
    return c1, c2
...
for gen in range(N_GENERATIONS):
    ...
    if len(feasible_idx) > 0:
        ...
        conv['min_cost'].append(np.inf)
        conv['max_coverage'].append(0)
        conv['min_variance'].append(np.inf)
    ...
    for fr in cfronts:
        if cnt + len(fr) <= POP_SIZE:
            for idx in fr:
                new_pop[cnt] = cp[idx]; no1[cnt] = co1[idx]
                no2[cnt] = co2[idx]; no3[cnt] = co3[idx]; nof[cnt] = cof[idx]
                cnt += 1
        else:
            rem = POP_SIZE - cnt
            fcrd = ccrowd[fr]
...
if len(pareto_indices) < 5:
    print(f'可行Pareto解不足({len(pareto_indices)}个)，放宽约束80%覆盖率...')
    cov_all = compute_coverage(*decode_variables(pop))
    pareto_indices = [i for i in final_fronts[0]
                      if np.all(cov_all[i] >= 0.80)]
