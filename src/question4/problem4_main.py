...
os.makedirs('problem4_output', exist_ok=True)
...
# Q1预测
try:
    df_pred = pd.read_excel('result/prediction_result.xlsx')
except:
    df_pred = pd.read_excel('prediction_result.xlsx')
...
df_dem.to_excel('problem4_output/未来需求推演.xlsx', index=False)
...
df_health.to_excel('problem4_output/健康度评价.xlsx', index=False)

df_exp = pd.DataFrame(all_exp)
df_exp.to_excel('problem4_output/动态扩容方案.xlsx', index=False)

df_pri = pd.DataFrame(all_priority)
df_pri = df_pri.sort_values('优先级评分', ascending=False)
df_pri.to_excel('problem4_output/扩容优先级.xlsx', index=False)

df_op = pd.DataFrame(all_op_cap)
df_op.to_excel('problem4_output/有效容量分析.xlsx', index=False)

df_sum = pd.DataFrame(final_summary)
df_sum.to_excel('problem4_output/情景对比汇总.xlsx', index=False)
...
pd.DataFrame(plan_recs).to_excel('problem4_output/未来三年扩展规划.xlsx', index=False)
...
with pd.ExcelWriter('problem4_output/problem4_final_result.xlsx') as writer:
...
plt.tight_layout();fig1.savefig('problem4_output/图1_多情景需求增长曲线.png');plt.close()
print('  图1: 多情景需求增长曲线')
...
plt.tight_layout();fig2.savefig('problem4_output/图2_健康度热力图.png');plt.close()
print('  图2: 健康度热力图')
...
plt.tight_layout();fig3.savefig('problem4_output/图3_调度有效容量对比.png');plt.close()
print('  图3: 调度有效容量对比')
...
plt.tight_layout();fig4.savefig('problem4_output/图4_扩容优先级排序.png');plt.close()
print('  图4: 扩容优先级排序')
...
plt.tight_layout();fig5.savefig('problem4_output/图5_三年动态扩容方案.png');plt.close()
print('  图5: 三年动态扩容方案')
...
print('\n' + '='*60)
print('输出文件清单:')
for f in sorted(os.listdir('problem4_output')):
    kb = os.path.getsize(f'problem4_output/{f}')/1024
    print(f'  {f} ({kb:.1f}KB)')
print('='*60)
