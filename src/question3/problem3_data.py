...
# 原始文件包含两个sheet，通过显式 sheet_name 分别读取
# =============================================================================
FILE_ATT3 = os.path.join(ROOT, 'data/raw/附件3 市主城区 10 区域分时段充电负荷.xlsx')
print('\n[1/4] 读取附件3...')

df3_wd = pd.read_excel(FILE_ATT3, sheet_name='工作日分时段充电负荷数据')
df3_wd = df3_wd.iloc[:10].copy()
df3_wd['日期类型'] = '工作日'

df3_we = pd.read_excel(FILE_ATT3, sheet_name='周末充电负荷数据（修改后）')
...
# =============================================================================
print('\n[2/4] 读取附件4...')
df4_raw = pd.read_excel(os.path.join(ROOT, 'data/raw/附件4 市主城区 10 个区域分时段电网最大允许负荷数据.xlsx'))
...
path_table = os.path.join(OUTPUT_DIR, '表_调度前分析.xlsx')
...
# 保存完整合并数据
df.to_pickle(os.path.join(OUTPUT_DIR, 'merged_data.pkl'))
