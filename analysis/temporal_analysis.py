...
from analysis.region_type_loader import get_region_names
...
def plot_temporal(hourly_analysis):
    ...
    plt.tight_layout()
    output_path = os.path.join(RESULT_DIR, 'figures', 'temporal.png')
...
def plot_weekday_weekend(daily_pivot, hourly_pivot):
    ...
    plt.tight_layout()
    output_path = os.path.join(RESULT_DIR, 'figures', 'weekday_weekend.png')
    fig.savefig(output_path, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
