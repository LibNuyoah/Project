...
def plot_correlation_heatmap(corr_matrix):
    ...
    plt.tight_layout()
    output_path = os.path.join(RESULT_DIR, 'figures', 'correlation_heatmap.png')
...
def plot_factor_bar(demand_corr):
    ...
    plt.tight_layout()
    output_path = os.path.join(RESULT_DIR, 'figures', 'factor_correlation.png')
    fig.savefig(output_path, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
...
def main():
    ...
    # 保存相关性矩阵
    corr_matrix.to_excel(os.path.join(RESULT_DIR, 'tables', 'correlation_matrix.xlsx'))
    print(f"\n✅ 相关性矩阵已保存: {os.path.join(RESULT_DIR, 'tables', 'correlation_matrix.xlsx')}")
